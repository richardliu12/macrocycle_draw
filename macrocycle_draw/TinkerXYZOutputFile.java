...
public class TinkerXYZOutputFile extends OutputFileFormat implements Immutable {
    ...
    */
        ...
        for (List<String> line : fileContents)
            {
                if (isFirstLine)
                ...
                else
                    {
                        ...
                        int tinkerAtomType = Integer.parseInt(line.get(5));
			            Atom newAtom = new Atom(line.get(1), position, tinkerAtomType);
			            atoms.add(newAtom);
    ...
    {
	    /*TinkerXYZOutputFile tinkerXYZOutputFile = new TinkerXYZOutputFile("arginine_test.xyz");
	    Molecule molecule = tinkerXYZOutputFile.molecule;
        System.out.println(molecule);
	    InputFileFormat.writeStringToDisk(molecule.toGaussianString(), "arginine_test.gjf");*/
        TinkerXYZOutputFile file = new TinkerXYZOutputFile("forcefield_jobs/geometry/ts1l1_r.xyz");
        System.out.println(file.molecule.name);
    }
